...
// ==/UserScript==
/*
  FL Tools Basic v2.2.8 — standalone dock (filters, soft-block, NSFW/SFW, Seen, navigation).
...
(function flToolsInit() {
  ...
  function buildJumpPanel() {
    ...
    FLMenuFooter.mount({ dock, row, jump, edition: FL_EDITION, version: FL_TOOLS_VERSION, studioLive: flStudioLive,
      diagnostics: () => {
        for (const [body, header] of [['fl-system-body','fl-system-header'],['fl-perf-body','fl-perf-header']]) {
          if (document.getElementById(body)?.classList.contains('fl-tool-hidden')) document.getElementById(header)?.click();
        }
        refreshDiagnostics();
        requestAnimationFrame(() => {
          const output = document.getElementById('fl-diagnostics-output');
          if (output) { output.tabIndex = -1; output.focus({preventScroll:true}); output.scrollIntoView({block:'nearest'}); }
        });
      }
    });
