...
2. **`@require` a tagged Basic asset** (never `latest`), e.g.  
   `https://github.com/Typical-Bits/fl-tools-basic/releases/download/v1.2.13/handoff.js`  
   Pin the tag that matches the shipped Basic CSS/handoff contract. The extra assets start appearing on the **next** bump-release after this split landed; until then vendor from this repo path.
...
Bump via **Actions → Bump release version**. No preview app. The bump does not need a `@version` change for CSS/handoff-only work; `sync-core` keeps the inlined userscript current on `main`.
